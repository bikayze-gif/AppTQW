                              ...
                              >
                                ...
                                <div className="mb-6 bg-slate-50 dark:bg-slate-700/30 rounded-lg p-4">
                                  <div className="text-center mb-4">
                                    <div className="text-4xl font-bold text-slate-900 dark:text-white font-mono tracking-tight">
                                      {(reminder.time.split(":")[0] || "12").padStart(2, "0")}:{(reminder.time.split(":")[1] || "00").padStart(2, "0")}
                                    </div>
                                  </div>
                                  
                                  <div className="flex items-stretch justify-center gap-1 bg-white dark:bg-slate-800 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700">
                                    {/* Hours Column */}
                                    <div className="flex-1 max-w-20">
                                      <div className="h-40 overflow-y-scroll scrollbar-hide flex flex-col">
                                        {Array.from({ length: 24 }).map((_, i) => {
                                          const hour = String(i).padStart(2, "0");
                                          const isSelected = hour === (reminder.time.split(":")[0] || "12").padStart(2, "0");
                                          return (
                                            <button
                                              key={i}
                                              onClick={() => {
                                                setReminder({ ...reminder, time: `${hour}:${reminder.time.split(":")[1] || "00"}` });
                                              }}
                                              className={`flex-1 py-2 px-1 text-sm font-medium transition-all ${
                                                isSelected
                                      ...
                                      <div className="text-xs text-center text-slate-500 dark:text-slate-400 py-2 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 font-semibold">
                                        HH
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex-1 max-w-20">
                                      <div className="h-40 overflow-y-scroll scrollbar-hide flex flex-col">
                                        {Array.from({ length: 60 }).map((_, i) => {
                                          ...
                                          const isSelected = minute === (reminder.time.split(":")[1] || "00").padStart(2, "0");
                                          return (
                                            <button
                                              key={i}
                                              onClick={() => {
                                                setReminder({ ...reminder, time: `${reminder.time.split(":")[0] || "12"}:${minute}` });
                                              }}
                                              className={`flex-1 py-2 px-1 text-sm font-medium transition-all ${
                                                isSelected
                                                  ? "bg-blue-500 text-white"
                          ...
                          <AnimatePresence>
                            {isLabelOpen && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, scale: 1, y: 0 }}
                                exit={{ opacity: 0, scale: 0.95, y: 10 }}
                                className="absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 p-4 w-64 z-50"
                              >
